...
questApp.controller('AppCtrl', ['$scope', '$http', '$location', 'CustomerService',
    function ($scope, $http, $location, CustomerService) {
        ...
        $scope.getCountryCities = function () {
            //get id country by value
            var id;
            var countries_list = $scope.countries;
            for (var index in countries_list){
                if (countries_list[index] == $scope.customer.Country){
                    id = index;
                }
            }
            if ($scope.customer.Country !== "") $scope.cities = CustomerService.getCountryCity(id, citylist, $scope.customer.Country);
